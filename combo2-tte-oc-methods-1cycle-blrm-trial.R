method_1cycle_blrm_trial <- list(
  ...
  {
    cohort_1cycle <- dplyr::select(cohort, -num_patients, -num_toxicities) %>%
      dplyr::mutate(
        num_patients = as.numeric(num_patients_1cycle),
        num_toxicities = as.numeric(num_toxicities_1cycle)
      )
    
    
    cohort_new <- cohort_1cycle %>% mutate(stratum_id="all")
    ...
    trial_configuration$current_date <- max(pmin(model$data$cycle2_caltime_dt, model$data$cens_caltime_dt))
  ...
  ) 
  {
    ...
    model$trial <- update(
      ...
      prior_EX_mu_mean_comp = matrix(
        c(
          logit(0.03), log(1.0), # hyper-mean of (intercept, log-slope) for drug A
          logit(0.04), log(1.0)  # hyper-mean of (intercept, log-slope) for SOC
        ),
      ...
      prior_EX_mu_sd_comp = matrix(
        c(
          1.0, log(4)/1.96*1.3, # sd of (intercept, log-slope) for drug A
          0.5, log(4)/1.96  # sd of (intercept, log-slope) for SOC
        ), 
        nrow = dims$num_components,
        ncol = 2,
        byrow = TRUE
